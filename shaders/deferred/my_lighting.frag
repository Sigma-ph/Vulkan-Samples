...
void main()
{
	...
	mediump vec3 pos     = world_w.xyz / world_w.w;
	vec4 albedo = subpassLoad(i_albedo);
	// Transform from [0,1] to [-1,1]
	mediump vec3 normal = subpassLoad(i_normal).xyz;
	normal      = normalize(2.0 * normal - 1.0);
	// Calculate lighting
	vec3 L = vec3(0.0);
	mediump vec4 MyVector = {1.0, 1.0, 1.0, 1.0};
	mediump float scaler = 2.0;
	for(int i=0; i<20; ++i){
		mediump vec4 test = {0.25, 0.25, 0.25, 0.25};
		MyVector += test;
		MyVector *= test;
	...
	{
		L += apply_point_light_medium(lights_info.point_lights[i], pos, normal);
	}
